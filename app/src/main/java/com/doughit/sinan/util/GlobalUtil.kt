...
object GlobalUtil {
    ...
    val appPackage: String
        get() = SiNanApplication.mContext.packageName
     ...
     */
    val appName: String
        get() = SiNanApplication.mContext.resources.getString(SiNanApplication.mContext.applicationInfo.labelRes)
    ...
    val appVersionName: String
        get() = SiNanApplication.mContext.packageManager.getPackageInfo(appPackage, 0).versionName
    ...
    val appVersionCode: Long
        get() = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.P) {
            SiNanApplication.mContext.packageManager.getPackageInfo(appPackage, 0).longVersionCode
        } else {
            SiNanApplication.mContext.packageManager.getPackageInfo(appPackage, 0).versionCode.toLong()
        }
    ...
    fun getDeviceSerial(): String {
        if (deviceSerial == null) {
            ...
            if ("google" != appChannel || "samsung" != appChannel) {
                try {
                    deviceId = Settings.Secure.getString(SiNanApplication.mContext.contentResolver, Settings.Secure.ANDROID_ID)
                } catch (e: Exception) {
                }
            ...
            var uuid = DataStoreUtil.readStringData("uuid", "")
            if (!TextUtils.isEmpty(uuid)) {
                deviceSerial = uuid
                return deviceSerial.toString()
            }
            uuid = UUID.randomUUID().toString().replace("-", "").toUpperCase(Locale.getDefault())
            CoroutineScope(Dispatchers.IO).launch { DataStoreUtil.saveStringData("uuid", uuid) }
            deviceSerial = uuid
            return deviceSerial.toString()
        } else {
            return deviceSerial.toString()
    ...
    fun getString(resId: Int): String {
        return SiNanApplication.mContext.resources.getString(resId)
    }
    ...
    fun getDimension(resId: Int): Int {
        return SiNanApplication.mContext.resources.getDimensionPixelOffset(resId)
    }
    ...
    fun getResourceId(name: String, type: String): Int {
        return SiNanApplication.mContext.resources.getIdentifier(name, type, appPackage)
    }
    ...
    fun getApplicationMetaData(key: String): String? {
        ...
        try {
            applicationInfo = SiNanApplication.mContext.packageManager.getApplicationInfo(appPackage, PackageManager.GET_META_DATA)
    ...
    fun isInstalled(packageName: String): Boolean {
        val packageInfo: PackageInfo? = try {
            SiNanApplication.mContext.packageManager.getPackageInfo(packageName, 0)
        } catch (e: PackageManager.NameNotFoundException) {
            null
        }
    ...
    fun getAppIcon(): Drawable {
        val packageManager = SiNanApplication.mContext.packageManager
